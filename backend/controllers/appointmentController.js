    ...
    try {
        ...
        if (released) {
            await prisma.appointment.update({
                ...
                data: { client_id, service_id: Number(service_id), status: 'pending', phone: smsPhone, send_sms_reminder: smsConsent, refusal_reason: null, is_slot_released: false, is_read: true },
            });
            appointmentId = released.id;
        } else {
            const appt = await prisma.appointment.create({
                data: { client_id, provider_id: Number(provider_id), service_id: Number(service_id), appointment_date: new Date(appointment_date), status: 'pending', phone: smsPhone, send_sms_reminder: smsConsent },
            });
...
module.exports = { createAppointment, getAvailability, getServices };
