    ...
    try {
        ...
        const rows = await prisma.$queryRaw`
            SELECT a.id, a.appointment_date, a.status, a.refusal_reason, a.is_read,
                   ...
                   (SELECT rating FROM reviews r WHERE r.appointment_id = a.id LIMIT 1) AS review_rating,
                   (SELECT comment FROM reviews r WHERE r.appointment_id = a.id LIMIT 1) AS review_comment
            FROM appointments a
            JOIN services s ON a.service_id = s.id
